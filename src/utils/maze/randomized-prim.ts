...
import { animateVisited } from "../animationUtils";
...
export default function randomizedPrim(graph: Graph): Set<Node> {
    ...
    while (wallList.length > 0) {
        ...
        const makePassage: boolean = randomWall.neighbors.filter((neighbor) => visited.has(neighbor)).length === 1;
        if (makePassage && randomWall.position[0] !== 0 && randomWall.position[0] !== rows - 1 && randomWall.position[1] !== 0 && randomWall.position[1] !== cols - 1) {
            randomWall.wall = false;
            walls.delete(randomWall);
            visited.add(randomWall);
            wallList.push(...randomWall.neighbors.filter((neighbor) => !visited.has(neighbor)));
        }
    }
